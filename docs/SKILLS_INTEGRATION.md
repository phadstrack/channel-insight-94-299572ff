...
## Audit Workflow with Skills → Mapeamento para 3 Ps

```
SETUP (Objetivos)
    ↓
DATA COLLECTION (Progressos base)
    ↓
ANALYSIS (Métricas, Progressos)
    ↓
TRACKING (Problemas de implementação)
    ↓
FINDINGS (Problemas consolidados)
    ↓
RECOMMENDATIONS (Plano prioritizado)
    ↓
REPORTING (3 Ps side-by-side)
```

**PROBLEMAS** ← TRACKING + FINDINGS (Fases 4–5)  
**PROGRESSOS** ← DATA COLLECTION + ANALYSIS (Fases 2–3)  
**PLANO** ← RECOMMENDATIONS + REPORTING (Fases 6–7)
