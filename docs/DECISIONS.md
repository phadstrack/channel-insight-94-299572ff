...
## ADR-012: Os 3 Ps como Entregável Central

**Status**: Accepted  
**Date**: 2026-05-16

### Context
ARC3 audita qualquer negócio, mas precisa de um modelo mental claro de "qual é o output de uma auditoria". 

Diferentes perspectivas:
- Executivos querem saber: o que está quebrado e o que fazer?
- Times operacionais querem saber: o que está funcionando e quanto esforço é cada fix?
- Leadership quer roadmap priorizado.

Nenhuma dessas é resolvida apenas com "relatório genérico de métricas".

### Decision
Toda auditoria ARC3 entrega explicitamente os **3 Ps**:

- **Problemas** — Findings com severity (críticos, altos, médios, baixos)
- **Progressos** — Métricas em trending positivo, canais healthy, insights
- **Plano** — Recomendações priorizadas por score (impacto + esforço)

Isso é o contrato de saída. Relatórios, dashboards, APIs — tudo organiza-se entorno dos 3 Ps.

### Consequences
- ✅ Modelo mental claro para todos (executivos, técnicos, stakeholders)
- ✅ Priorização natural (Problemas primeiro, depois Progressos a manter, depois Plano)
- ✅ Dashboard unificado mostra os 3 Ps lado a lado
- ✅ Fácil comunicar valor de ARC3 para prospects
- ❌ Menos flexibilidade em formatos de relatório (tudo volta aos 3 Ps)
- ❌ Precisa treinar as 10 skills a pensar em termos de 3 Ps

### Mapeamento nos Layers
- **Problemas** ← Findings Layer + Tracking Layer (severidade crítica/alta/média/baixa)
- **Progressos** ← Analysis Layer (métricas positivas, trending em alta)
- **Plano** ← Recommendations Layer (scored, com owner, steps)

---

## Próximas ADRs a Considerar

- ADR-013: Cache strategy (Redis vs DB)
- ADR-014: Async job processing (Bull, Resque, etc)
- ADR-015: Multi-tenancy (por cliente ou por workspace)
- ADR-016: Versionamento de recomendações
- ADR-017: Integração com Slack/Teams para alertas
- ADR-018: Export de dados em formatos específicos (PowerPoint, PDF)
